...
export default function TicketsResultPage() {
  ...
  useEffect(() => {
    const loadTickets = async () => {
      try {
        ...
        if (!queryData) {
          router.push(`/${cliente}/meus-bilhetes`)
          return
        }
  ...
  if (!ticketData) {
    return (
      <div className="min-h-screen bg-cinza-claro dark:bg-cinza-escuro">
        <div className="max-w-2xl mx-auto px-4 py-8">
          <a href={`/${cliente}/meus-bilhetes`} className=" items-center gap-2 text-azul-royal dark:text-amarelo-claro text-1xl font-bold inline-flex transition">
            <ArrowLeft className="w-5 h-5" />
            Voltar
          </a>
